...
async function copyImpl(
  ...
  if (opts.linkStrategy?.type !== `HardlinkFromIndex` || !sourceStat.isFile()) {
    if (
      updated || destinationStat?.mtime?.getTime() !== mtime.getTime() ||
...
async function copyFileViaIndex(
  ...
  if (destinationStat) {
    const isDestinationHardlinkedFromIndex = indexStat &&
  ...
  const tempPath = !indexStat && atomicBehavior === 1 /* Rename */
    ? `${indexPath}.${
...
class NodeFS extends BasePortableFakeFS {
  ...
  readdirSync(p, opts) {
    if (opts) {
      if (opts.recursive && process.platform === `win32`) {
        if (opts.withFileTypes) {
          return this.realFs.readdirSync(npath.fromPortablePath(p), opts).map(
            direntToPortable,
          )
        } else {
          return this.realFs.readdirSync(npath.fromPortablePath(p), opts).map(
...
const URL = Number(process.versions.node.split(".", 1)[0]) < 20
  ? URL$1
  : globalThis.URL
...
const HAS_LAZY_LOADED_TRANSLATORS = major === 20 && minor < 6 ||
  major === 19 && minor >= 3
const SUPPORTS_IMPORT_ATTRIBUTES = major >= 21 || major === 20 && minor >= 10 ||
  major === 18 && minor >= 20
...
async function load$1(urlString, context, nextLoad) {
  ...
  if (format === `json`) {
    if (SUPPORTS_IMPORT_ATTRIBUTES_ONLY) {
      ...
      const type = `importAttributes` in context
...
async function resolve$1(originalSpecifier, context, nextResolve) {
  ...
  const issuer = parentURL && tryParseURL(parentURL)?.protocol === `file:`
...
if (!HAS_LAZY_LOADED_TRANSLATORS) {
  ...
  if (originalReadFile) {
    ...
    binding2.fstat = function (...args) {
      ...
      if (
        (fd & ZIP_MASK) === ZIP_MAGIC && useBigint === false && req === void 0
      ) {
        try {
