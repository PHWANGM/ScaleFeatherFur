...
import {
  ...
  type SpeciesTarget,
} from "../lib/db/repos/species.targets.repo"
  ...
  const needs: NeedCard[] = useMemo(() => {
    ...
    const uvbHours =
      fmtHoursRange(target.uvb_daily_hours_min, target.uvb_daily_hours_max, {
        ...
        daysLabel: (_d, fixed) =>
          t("speciesNeeds.units.daysFloat", { value: fixed }),
      }) ??
        fmtHoursRange(
          target.photoperiod_hours_min,
          target.photoperiod_hours_max,
          {
            hoursLabel: (h) => t("speciesNeeds.units.hours", { count: h }),
            daysLabel: (_d, fixed) =>
              t("speciesNeeds.units.daysFloat", { value: fixed }),
    ...
    const uvbIntensity =
      target.uvb_intensity_min != null || target.uvb_intensity_max != null
        ? `${target.uvb_intensity_min ?? target.uvb_intensity_max}-${
          target.uvb_intensity_max ?? target.uvb_intensity_min
        }${uvbUnit}`
        : null
    ...
    if (uvbHours || uvbIntensity) {
      cards.push({
        ...
        subtitle: [
          uvbHours
            ? t("speciesNeeds.cards.uvb.perDay", { range: uvbHours })
            : null,
          uvbIntensity
            ? t("speciesNeeds.cards.uvb.intensity", { value: uvbIntensity })
            : null,
        ]
          .filter(Boolean)
          .join(" · "),
            ...
            <View style={[styles.card, { backgroundColor: "#FFF6E5" }]}>
              <View style={styles.textWrap}>
                <Text style={styles.cardTitle}>
                  {t("speciesNeeds.empty.title")}
                </Text>
                <Text style={styles.cardSubtitle}>
                  {t("speciesNeeds.empty.subtitle")}
                </Text>
              </View>
