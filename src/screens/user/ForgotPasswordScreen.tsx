      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
          <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}>
            <Text style={[styles.title, { color: palette.text }]}>{t('forgotPassword.title')}</Text>
            <Text style={[styles.sub, { color: palette.subText }]}>{t('forgotPassword.subtitle')}</Text>

            <Text style={[styles.label, { color: palette.text }]}>{t('forgotPassword.emailLabel')}</Text>
            ...
            >
              <Text style={[styles.btnText, { color: palette.bg }]}>
                {loading ? t('forgotPassword.actions.sending') : t('forgotPassword.actions.send')}
              </Text>
            </Pressable>
          </View>
        </ScrollView>
