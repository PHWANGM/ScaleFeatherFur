      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
          <View style={styles.header}>
            ...
            <Text style={[styles.title, { color: palette.text }]}>{t('login.title')}</Text>
            <Text style={[styles.subtitle, { color: palette.subText }]}>{t('login.subtitle')}</Text>
          </View>

          <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}>
            <Text style={[styles.label, { color: palette.text }]}>{t('login.emailLabel')}</Text>
            ...
            <Pressable onPress={() => navigation.navigate('ForgotPassword')} style={styles.forgotBtn}>
              <Text style={[styles.forgotText, { color: palette.subText }]}>{t('login.forgot')}</Text>
            </Pressable>
            ...
            <Pressable onPress={() => navigation.navigate('Signup')} style={styles.linkBtn}>
              <Text style={[styles.linkText, { color: palette.subText }]}>{t('login.links.toSignup')}</Text>
            </Pressable>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  forgotBtn: { marginTop: theme.spacing.sm, alignItems: 'flex-end', paddingVertical: theme.spacing.xs },
