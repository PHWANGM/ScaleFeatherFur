...
export default function AuthCallbackScreen() {
  ...
  const status: 'loading' | 'ok' | 'error' =
    phase === 'success' ? 'ok' : phase === 'error' || phase === 'noLink' ? 'error' : 'loading';
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}>
          ...
          {(status === 'error' || status === 'ok') && (
            <Pressable style={[styles.btn, { backgroundColor: palette.primary }]} onPress={onPrimaryPress}>
              <Text style={[styles.btnText, { color: palette.bg }]}>
                {status === 'error' ? t('authCallback.actions.backToLogin') : t('authCallback.actions.enterApp')}
              </Text>
            </Pressable>
          )}
        </View>
