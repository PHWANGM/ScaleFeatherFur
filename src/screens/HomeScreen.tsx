...
import { selectCurrentPetId, setCurrentPetId } from "../state/slices/petsSlice"
import {
...
export default function HomeScreen({ navigation }: Props) {
  ...
  const { coords, locationName, loading: locationLoading } =
    useCurrentLocation()
  ...
  const refreshDailyTasks = useCallback(async () => {
    ...
    try {
      ...
      // 3) fallback: Local DB
      const localRows = await DbTasks.getDailyTaskStatus(
        currentPetId,
        dayStartISO,
        dayEndISO,
      )
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: palette.bg }]}>
        ...
        <Pressable
          ...
          onPress={() =>
            Alert.alert(t("settings.title"), t("home.openSettings"))}
          hitSlop={10}
        >
          <Feather
            name="settings"
