...
export async function openDatabaseAsync(
  ...
  throw new Error(
    "openDatabaseAsync called in unit test. Use __setTestDB to inject a mock DB.",
  )
}
...
export function createTestDB(): SQLiteDatabase {
  ...
  async function execAsync(sql?: string) {
    ...
    for (const s of stmts) {
      ...
      if (upper.startsWith("CREATE TABLE IF NOT EXISTS")) {
        ...
        if (table === "care_logs" && !tables.care_logs) {
          tables.care_logs = new Map()
        }
      }
    }
  }
  ...
  async function runAsync(
    ...
    if (up.startsWith("INSERT INTO SPECIES")) {
      // VALUES (?, ?, ?, ?, ?, ?)
      const [key, common_name, scientific_name, notes, created_at, updated_at] =
        params
      const existed = tables.species.has(key)
      tables.species.set(key, {
        key,
        common_name,
        scientific_name,
        notes,
    ...
    if (up.startsWith("INSERT INTO PETS")) {
      ...
      if (!tables.species.has(species_key)) {
        throw new Error(
          "FOREIGN KEY constraint failed: pets.species_key → species.key",
        )
      }
      // 模擬 CHECK(habitat IN ...)
    ...
    if (up.startsWith("UPDATE PETS SET")) {
      ...
      // 解析簡單的 "col = ?, col2 = ?" 清單
      const setPart = sql.replace(/^[\s\S]*?SET\s+/i, "").replace(
        /\s+WHERE[\s\S]*$/i,
        "",
      )
      ...
      // 驗證 species_key（若有）
      if (
        patch.species_key !== undefined &&
        !tables.species.has(patch.species_key)
      ) {
        throw new Error(
          "FOREIGN KEY constraint failed: pets.species_key → species.key",
        )
    ...
    if (up.startsWith("INSERT INTO CARE_LOGS")) {
      ...
      if (!tables.pets.has(pet_id)) {
        throw new Error(
          "FOREIGN KEY constraint failed: care_logs.pet_id → pets.id",
        )
      }
      const allowedType = [
        "feed",
  ...
  async function getAllAsync<T = any>(
    ...
    if (up.startsWith("SELECT * FROM PETS")) {
      ...
      if (hasSpecies) {
        ...
        rows = rows.filter((r) => r.species_key === val)
      }
      if (hasHabitat) {
        const val = params[pIndex++]
        rows = rows.filter((r) => r.habitat === val)
      }
      if (hasNameLike) {
        const val = params[pIndex++]
        rows = rows.filter((r) => likeMatch(r.name ?? "", String(val)))
      }
