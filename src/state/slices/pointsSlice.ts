...
import { execute, query } from "../../lib/db/db.client"
...
export const addPointsForTask = createAsyncThunk(
  "points/addForTask",
  async (p: { petId: string; taskKey: string; points: number }) => {
    // 取得最新餘額
    ...
    const rows = await query<Ledger>(`SELECT * FROM points_ledger WHERE id=?`, [
      id,
    ])
    return rows[0]
  },
