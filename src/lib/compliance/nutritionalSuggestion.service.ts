...
import {
  ...
  type SpeciesTarget,
} from "../db/repos/species.targets.repo"
import { getPetWithSpeciesById } from "../db/repos/pets.repo"
import type { FoodAnalysisResult, FoodType } from "../ai/gemini.service"
  ...
  switch (foodType) {
    case "vegetables": {
      ...
      if (min >= 70) {
        return {
          ok: true,
          message: `${speciesName}是草食性動物，${label}是主食`,
        }
      }
    ...
    case "meat": {
      ...
      if (vegMin >= 80 && meatMax <= 10) {
        return {
          ok: false,
          message:
            `⚠️ ${speciesName}主要是草食性，蛋白質來源應限制在 ${meatMax}% 以內`,
        }
      }
    ...
    case "fruit": {
      ...
      if (fruitMax <= 10) {
        return {
          ok: false,
          message:
            `⚠️ 水果對${speciesName}來說應作為點心，建議控制在 ${fruitMax}% 以內`,
        }
      }
  ...
  switch (foodType) {
    ...
    case "meat":
      ...
      if (vegMin >= 80 && meatMax <= 10) {
        return {
          warning:
            `${speciesName}是草食性動物，蛋白質攝取應控制在 ${meatMax}% 以內`,
    ...
    case "fruit":
      ...
      if (fruitMax <= 10) {
        return {
          warning:
            `水果含糖量高，對${speciesName}來說應作為點心，建議控制在 ${fruitMax}% 以內`,
          isOk: false,
        }
      }
      if (fruitMax <= 20) {
