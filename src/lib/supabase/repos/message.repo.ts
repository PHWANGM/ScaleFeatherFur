...
export function toConversationListItems(rows: ConversationSummaryRow[]): ConversationListItem[] {
  return rows.map((r) => {
    ...
    return {
      ...
      displayTitle: r.is_group ? (r.title ?? 'Group') : (r.other_display_name ?? 'Unknown'),
      preview: r.last_message_text ?? (r.last_message_at ? '(no preview)' : 'Start a conversation'),
...
// ===== List =====
export async function fetchConversationSummaries(myId: string): Promise<ConversationSummaryRow[]> {
 ...
 */
export async function createOrGetDm(otherUserId: string): Promise<{ conversationId: string; title?: string } | null> {
  ...
  try {
    const { data: p, error: pErr } = await supabase.from('profiles').select('display_name').eq('id', otherUserId).single();
...
export async function sendMessage(params: { conversationId: string; senderId: string; body: string }): Promise<MessageRow | null> {
...
export function subscribeToConversationMessages(params: { conversationId: string; onInsert: (msg: MessageRow) => void }) {
