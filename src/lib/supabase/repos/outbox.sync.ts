...
import { listUnsyncedOutbox, markOutboxFailed, markOutboxSynced } from '../../db/repos/sync.outbox.repo';
  ...
  for (const row of rows) {
    try {
      if (row.type === 'task_complete') {
        const payload = JSON.parse(row.payload_json) as { petId: string; taskKey: string; day: string };
        await rpcCompleteTask(supabase, payload.petId, payload.taskKey, payload.day);
