...
function parseTargetRow(r: SpeciesTargetRowRaw): SpeciesTarget {
  ...
  const d3_days_min = typeof r.vitamin_d3_interval_hours_min === "number"
    ? Math.round(r.vitamin_d3_interval_hours_min / 24)
    : null

  const d3_days_max = typeof r.vitamin_d3_interval_hours_max === "number"
  ...
  // 鈣粉頻率：直接吃欄位
  const calcium_every_meals = typeof r.calcium_every_meals === "number"
...
export async function upsertSpeciesTarget(
  ...
  // 以天 -> 以小時（存回 DB 小時欄位）
  const d3hMin = input.vitamin_d3_interval_days_min != null
    ? input.vitamin_d3_interval_days_min * 24
    : null
  const d3hMax = input.vitamin_d3_interval_days_max != null
