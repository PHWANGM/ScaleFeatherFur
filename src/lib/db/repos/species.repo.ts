...
export async function getSpeciesByKey(key: string): Promise<SpeciesRow | null> {
  const rows = await query<SpeciesRow>(
    `SELECT * FROM species WHERE key = ? LIMIT 1`,
    [key],
  )
  return rows[0] ?? null
}
...
export async function insertSpecies(input: {
  ...
  const rows = await query<SpeciesRow>(`SELECT * FROM species WHERE key = ?`, [
    input.key,
  ])
  return rows[0]!
}
...
export async function updateSpecies(key: string, patch: {
  ...
  if (patch.scientific_name !== undefined) {
    fields.push("scientific_name = ?")
    params.push(patch.scientific_name)
  }
  ...
  params.push(updated)
  await execute(`UPDATE species SET ${fields.join(", ")} WHERE key = ?`, [
    ...params,
    key,
  ])
  const rows = await query<SpeciesRow>(`SELECT * FROM species WHERE key = ?`, [
...
export async function debugSpeciesSnapshot(): Promise<{
  ...
  sample: Pick<
    SpeciesRow,
    "key" | "common_name" | "scientific_name" | "updated_at"
  >[]
}> {
  ...
  const sample = await query<
    Pick<SpeciesRow, "key" | "common_name" | "scientific_name" | "updated_at">
  >(
