  ...
  useEffect(() => {
    ...
    const onShow = (e: any) => {
      ...
      containerRef.current?.measureInWindow((x, y, width, height) => {
        ...
        const overlap = inputBottomY - (keyboardTopY - androidCandidateBar - extraGap);
        if (overlap > 0) {
    ...
    >
      <View style={[styles.inputWrapper, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <Pressable
          ...
          style={[
            styles.sendButton,
            { backgroundColor: canSend ? colors.primary : colors.border, opacity: sending ? 0.8 : 1 },
          ]}
        >
          {sending ? <ActivityIndicator size="small" color="#122017" /> : <Text style={styles.sendButtonText}>發送</Text>}
        </Pressable>
      </View>
    </Animated.View>
  );
...
const styles = StyleSheet.create({
  inputContainer: { paddingHorizontal: theme.spacing.md, paddingTop: theme.spacing.sm, borderTopWidth: 1 },
  inputWrapper: { flexDirection: 'row', alignItems: 'flex-end', borderRadius: 24, borderWidth: 1, paddingLeft: 12, paddingRight: 6, paddingVertical: 6 },
  textInput: { flex: 1, paddingVertical: 8, maxHeight: 110, fontSize: 16 },
  sendButton: { width: 56, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', marginLeft: 8, marginBottom: 2 },
