...
const ensureISODateTime = (isoLike: string) =>
  isoLike.includes("T")
    ? isoLike
    : `${isoLike}T${new Date().toISOString().split("T")[1]}`
...
const CustomCalendar: React.FC = () => {
  ...
  const effectiveSelectedISO = useMemo(() => selectedDate ?? nowIso(), [
    selectedDate,
  ])
  ...
  const setSelectedDateAndMark = useCallback(
    (dateISODate: string) => {
      ...
      console.log(
        "[CustomCalendar] onDayPress -> dispatch setSelectedDate:",
        nextSelected,
      )
  ...
  const debugScanLogs = useCallback(async (petId: string | null) => {
    try {
      const total = await query<{ c: number }>(
        "SELECT COUNT(*) AS c FROM care_logs",
        [],
      )
      ...
      if (petId) {
        ...
        console.log(
          "[Debug] pet min_at =",
          minMaxByPet[0]?.min_at,
          ", max_at =",
          minMaxByPet[0]?.max_at,
        )
      }
    } catch (e) {
  ...
  const fetchAndMark = useCallback(async () => {
    ...
    try {
      ...
      if (!selectedDate) {
        const now = nowIso()
        console.log(
          "[CustomCalendar] selectedDate is empty, init with nowIso =",
          now,
        )
