...
const StatsContainer: React.FC = () => {
  ...
  const dateISO = useMemo(() => selectedDate ?? new Date().toISOString(), [
    selectedDate,
  ])
  ...
  const uvbDataStr = useMemo(() => {
    if (!report) return "0/—"
    const actual = isFinite(report.uvb?.actualHours ?? 0)
      ? (report.uvb?.actualHours ?? 0)
      : 0
  ...
  const d3DataStr = useMemo(() => {
    ...
    const targetStr = parseWeeklyTargetCount(
      report.supplements?.d3PerWeekRule ?? null,
    )
