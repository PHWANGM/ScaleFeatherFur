...
async function generateWithGeminiFixed(params: {
  ...
  const url = `https://generativelanguage.googleapis.com/${v}/models/${model}:generateContent?key=${params.apiKey}`;
  ...
  if (!resp.ok) {
    return {
      ok: false,
      status: resp.status,
      bodyText: `Gemini generate failed (fixed model=${model}, api=${v}). status=${resp.status} body=${text}`,
    };
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const GEMINI_MODEL = (Deno.env.get("GEMINI_MODEL") ?? "gemini-3-pro-preview").trim();
    const GEMINI_API_VERSION = (Deno.env.get("GEMINI_API_VERSION") ?? "v1beta").trim();
