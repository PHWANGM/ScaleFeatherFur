...
//
// ✅ Fixed to Gemini 3 Pro Preview (v1beta) only
// ✅ Works with: supabase functions deploy ai-analyze --no-verify-jwt
// ✅ We still manually verify the user using Authorization header (supabase auth.getUser)
//
// Env required (set in Supabase project secrets):
// - SUPABASE_URL
// - SUPABASE_ANON_KEY
...
// - GEMINI_API_VERSION (default: "v1beta")  // keep as v1beta for preview models
//
// Tables used (same as your current code):
// - public.ai_jobs(id, user_id, image_path, status, model, updated_at, error, ...)
// - public.ai_results(job_id, result_json, created_at, ...)
// Storage bucket:
// - ai-images
//
// Notes:
// - If gemini-3-pro-preview isn't available for your key/project, you'll still get 404.
//   In that case, your key/project simply doesn't have access to that model.
...
type MinimalClient = {
  ...
  auth: {
    getUser(): Promise<
      { data: { user: { id: string } | null }; error: DbError | null }
    >
  }
}
...
/* =========================
   ✅ Gemini fixed-model call (v1beta)
   ========================= */

async function generateWithGeminiFixed(params: {
  apiKey: string
...
> {
  const v = params.apiVersion.trim() || "v1beta"
  const model = params.model.trim()

  // v1beta is required for many preview models; v1 may 404.
  const url =
    `https://generativelanguage.googleapis.com/${v}/models/${model}:generateContent?key=${params.apiKey}`
  ...
  if (!resp.ok) {
    return {
      ...
      status: resp.status,
      bodyText:
        `Gemini generate failed (fixed model=${model}, api=${v}). status=${resp.status} body=${text}`,
    }
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const GEMINI_MODEL =
      (Deno.env.get("GEMINI_MODEL") ?? "gemini-3-pro-preview").trim()
      ...
      return json(
        {
          error: "Missing env",
          detail:
            "Require SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY, GEMINI_API_KEY",
        },
        500,
      )
    }
    ...
    if (!GEMINI_MODEL) {
      return json({ error: "Missing GEMINI_MODEL" }, 500)
    }
    ...
    // ✅ no-verify-jwt：由這裡驗證身份（不要讓 function gateway 驗）
    ...
    try {
      ...
      job_id = typeof body["job_id"] === "string"
    ...
    const imageResp = await fetchWithTimeout(signed.signedUrl, {
      method: "GET",
    }, 15_000)
    if (!imageResp.ok) {
      await markJob(service, job_id, {
    ...
    if (bytes.byteLength > MAX_IMAGE_BYTES) {
      const msg =
        `Image too large: ${bytes.byteLength} bytes (limit ${MAX_IMAGE_BYTES}). Please resize/compress before upload.`
      await markJob(service, job_id, {
        status: "failed",
...
`.trim()
    ...
    const gem = await generateWithGeminiFixed({
    ...
    if (!gem.ok) {
      await markJob(service, job_id, {
        status: "failed",
        error: gem.bodyText.slice(0, 900),
      })
    ...
    try {
      ...
      parsed = {
        ...
        estimatedWeightGrams: Math.max(
          0,
          Math.round(Number(obj["estimatedWeightGrams"]) || 0),
        ),
        identifiedItems: Array.isArray(obj["identifiedItems"])
          ? (obj["identifiedItems"] as unknown[]).filter((x) =>
            typeof x === "string"
          ).slice(0, 6) as string[]
          : [],
